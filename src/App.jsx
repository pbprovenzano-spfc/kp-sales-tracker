...
// ─── INITIAL DATA ─────────────────────────────────────────────────────────────
const ADMIN = { id: "admin", username: "admin", password: "1234", isAdmin: true, companyIds: [] };

const makeClients = () => [
  {
    id: 1, name: "Grupo Leão", year: 2026, annualGoal: 871780.88,
    quarters: [
      { label: "1º Trimestre", goal: 219115.96, realized: 63248.0, lastUpdate: "2026-03-01T10:30:00" },
      { label: "2º Trimestre", goal: 217945.22, realized: 0, lastUpdate: null },
      { label: "3º Trimestre", goal: 214359.85, realized: 0, lastUpdate: null },
      { label: "4º Trimestre", goal: 220359.85, realized: 0, lastUpdate: null },
    ],
  },
  {
    id: 2, name: "Empresa Alpha", year: 2026, annualGoal: 540000.0,
    quarters: [
      { label: "1º Trimestre", goal: 130000.0, realized: 42000.0, lastUpdate: "2026-02-28T09:15:00" },
      { label: "2º Trimestre", goal: 135000.0, realized: 0, lastUpdate: null },
      { label: "3º Trimestre", goal: 135000.0, realized: 0, lastUpdate: null },
      { label: "4º Trimestre", goal: 140000.0, realized: 0, lastUpdate: null },
    ],
  },
  {
    id: 3, name: "Rede Beta", year: 2026, annualGoal: 1200000.0,
    quarters: [
      { label: "1º Trimestre", goal: 295000.0, realized: 108000.0, lastUpdate: "2026-03-05T14:00:00" },
      { label: "2º Trimestre", goal: 305000.0, realized: 0, lastUpdate: null },
      { label: "3º Trimestre", goal: 300000.0, realized: 0, lastUpdate: null },
      { label: "4º Trimestre", goal: 300000.0, realized: 0, lastUpdate: null },
    ],
  },
];
...
const BS = {
  ...
  boxShadow: "0 4px 20px #e5393533", fontFamily: "'DM Sans', sans-serif",
};
...
function ProgressBar({ value, thin, print }) {
  if (print) return (
    <div className="print-bar-bg">
      <div className="print-bar-fill" style={{ width: `${value}%`, background: "#16a34a" }} />
    </div>
  );
  return (
...
function Sidebar({ currentUser, screen, onNav, onLogout }) {
  const navItems = [
    ...
    ...(currentUser.isAdmin ? [{ id: "kpi", icon: "📊", label: "Painel KPIs" }] : []),
  ];
  return (
    <div style={{
      width: 220, minHeight: "100vh", background: "#0a0a0a",
      borderRight: "1px solid #181818", display: "flex", flexDirection: "column",
      position: "fixed", top: 0, left: 0, zIndex: 50, fontFamily: "'DM Sans', sans-serif",
    }}>
      {/* Logo area */}
...
function UserModal({ users, clients, onAdd, onDelete, onUpdateUser, onUpdateUserCompanies, onClose }) {
  ...
  const [editName, setEditName] = useState(""); const [editPass, setEditPass] = useState(""); const [editErr, setEditErr] = useState("");
  ...
  const add = () => {
    ...
    const n = nu.trim(); setNu(""); setNp(""); setErr("");
    setOk(`Usuário "${n}" criado!`); setTimeout(() => setOk(""), 2500);
  };
  ...
  const saveEdit = (u) => {
    ...
    setOk("Usuário atualizado!"); setTimeout(() => setOk(""), 2500);
  };
  ...
  const togglePanel = (uid, mode) => {
    ...
    if (mode === "edit") { openEdit(users.find(u => u.id === uid)); return; }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000c", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, fontFamily: "'DM Sans', sans-serif" }} onClick={onClose}>
      <div style={{ background: "#141414", borderRadius: 20, padding: 28, width: 380, border: "1px solid #282828", maxHeight: "88vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
          ...
          <div key={u.id} style={{ background: "#1c1c1c", borderRadius: 12, marginBottom: 8, border: "1px solid #282828", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px" }}>
              <div>
                ...
                <div style={{ color: "#555", fontSize: 11 }}>{u.isAdmin ? "Administrador" : `${(u.companyIds || []).length} empresa(s)`}</div>
              </div>
            ...
            {expandedUser?.id === u.id && expandedUser?.mode === "edit" && (
              <div style={{ borderTop: "1px solid #282828", padding: 14, background: "#161616" }}>
                <div style={{ fontSize: 10, color: "#555", marginBottom: 10, letterSpacing: 1 }}>EDITAR USUÁRIO</div>
                <FL>Novo nome</FL>
        ...
        <FL>Usuário</FL>
        <input value={nu} onChange={e => { setNu(e.target.value); setErr(""); }} placeholder="nome" style={IS} />
        <FL mt={10}>Senha</FL>
...
function CompanyModal({ existing, onSave, onClose }) {
  ...
  const computedGoals = () => {
    ...
    const qv = qs.map(parseNum); return { annualGoal: qv.reduce((s, x) => s + x, 0), q: qv };
  };
  const submit = () => {
        ...
        {mode === "annual" ? (
          ...
          <>
            {["1º Trimestre", "2º Trimestre", "3º Trimestre", "4º Trimestre"].map((l, i) => (
              <div key={l}><FL mt={12}>{l} (R$)</FL><input value={qs[i]} onChange={e => setQ(i, e.target.value)} placeholder="ex: 125000" style={{ ...IS, marginTop: 5 }} /></div>
            ))}
            {annualGoal > 0 && (
...
function ClientList({ clients, currentUser, users, isMobile, onSelect, onSaveClient, onDeleteClient, onAddUser, onDeleteUser, onUpdateUser, onUpdateUserCompanies, onLogout, onKpi }) {
  ...
  const inner = (
    <div style={{ padding: isMobile ? "18px 20px 80px" : "28px 36px 60px" }}>
      {/* KPI button — mobile only (desktop uses sidebar) */}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {isMobile && currentUser.isAdmin && (
            <button onClick={() => setShowUsers(true)} style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 10, color: "#888", fontSize: 12, padding: "7px 12px", cursor: "pointer", fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>👥</button>
          )}
          {!isMobile && currentUser.isAdmin && (
            <button onClick={() => setShowUsers(true)} style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 10, color: "#aaa", fontSize: 12, padding: "8px 14px", cursor: "pointer", fontWeight: 600, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>👥 Usuários</button>
          )}
        </div>
      </div>
      ...
      {!isMobile ? (
        <div style={{ background: "#111", borderRadius: 16, border: "1px solid #1a1a1a", overflow: "hidden" }}>
          ...
          {visibleClients.length === 0 && (
            <div style={{ textAlign: "center", padding: "50px 20px", color: "#444", fontSize: 14 }}>Nenhuma empresa cadastrada</div>
          )}
          {visibleClients.map((c, idx) => {
            const total = c.quarters.reduce((s, q) => s + q.realized, 0);
            const p = pct(total, c.annualGoal);
            return (
              <div key={c.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.5fr 100px 120px", padding: "14px 20px", borderBottom: idx < visibleClients.length - 1 ? "1px solid #161616" : "none", alignItems: "center", transition: "background .15s" }}
                onMouseEnter={e => e.currentTarget.style.background = "#161616"}
                onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                <div style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }} onClick={() => onSelect(c)}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#e53935,#c62828)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{c.name[0]}</div>
                  <div>
                ...
                {currentUser.isAdmin && (
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button onClick={() => setDeleteClient(c)} style={{ padding: "5px 9px", background: "#1a0a0a", border: "1px solid #e5393533", borderRadius: 7, color: "#ff6f61", fontSize: 11, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>🗑️</button>
                  </div>
                )}
              </div>
            );
            ...
            return (
              <div key={c.id} style={{ background: "#111", borderRadius: 16, padding: "16px 18px", marginBottom: 10, border: "1px solid #1a1a1a" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14, cursor: "pointer" }} onClick={() => onSelect(c)}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#e53935,#c62828)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>{c.name[0]}</div>
                ...
                {currentUser.isAdmin && (
                  <div style={{ display: "flex", gap: 8, marginTop: 12, paddingTop: 12, borderTop: "1px solid #1a1a1a" }}>
                    ...
                    <button onClick={() => setDeleteClient(c)} style={{ flex: 1, padding: "7px 0", background: "#1a0a0a", border: "1px solid #e5393533", borderRadius: 8, color: "#ff6f61", fontSize: 12, cursor: "pointer", fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>🗑️ Excluir</button>
                  </div>
                )}
              </div>
            );
...
function Dashboard({ client, isMobile, onBack, onUpdate }) {
  ...
  const annualSummaryDesktop = !isMobile && (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 8 }}>
      {[
        { label: "Meta Anual", value: fmt(client.annualGoal), color: "#888", border: "#1a1a1a" },
        { label: "Realizado Anual", value: fmt(total), color: GREEN, border: GREEN + "28", extra: `${annualPct}% atingido` },
        { label: "Faltante Anual", value: fmt(annualRem), color: annualRem === 0 ? GREEN : RED, border: RED + "28" },
      ].map(card => (
        <div key={card.label} className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", border: `1px solid ${card.border}` }}>
          <div style={{ fontSize: 10, color: "#555", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 6 }}>{card.label}</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: card.color, fontFamily: "monospace" }}>{card.value}</div>
          {card.extra && (
            <>
              <ProgressBar value={annualPct} />
              <span style={{ fontSize: 11, color: GREEN, fontWeight: 700 }}>{card.extra}</span>
            </>
          )}
        </div>
      ))}
  ...
  const quarterCards = client.quarters.map((q, i) => {
    ...
    if (!isMobile) {
      // Desktop: one row per quarter with 3 columns
      return (
        <div key={i}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 8 }}>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", border: "1px solid #1a1a1a" }}>
              ...
              <div className="print-value-dark" style={{ fontSize: 18, fontWeight: 700, color: "#f0f0f0", fontFamily: "monospace" }}>{fmt(q.goal)}</div>
            </div>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", border: `1px solid ${GREEN}28`, position: "relative" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 11, color: GREEN, fontWeight: 700 }}>{qPct}%</span>
                {q.lastUpdate && <span style={{ fontSize: 9, color: "#2a2a2a", fontStyle: "italic" }}>atualizado {fmtDate(q.lastUpdate)}</span>}
              </div>
              <button className="no-print" onClick={() => setModal(i)} style={{ position: "absolute", top: 14, right: 14, background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 8, color: "#888", fontSize: 11, fontWeight: 700, padding: "5px 9px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = "#e53935"; e.currentTarget.style.color = "#ff6f61"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "#2a2a2a"; e.currentTarget.style.color = "#888"; }}>
                ✏️
              </button>
            </div>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", border: `1px solid ${RED}28` }}>
              <div style={{ fontSize: 10, color: "#666", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 6 }}>Faltante {q.label}</div>
              <div className="print-value-red" style={{ fontSize: 18, fontWeight: 700, color: qRem === 0 ? GREEN : RED, fontFamily: "monospace" }}>{fmt(qRem)}</div>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div key={i}>
        <SectionTitle>{q.label.toUpperCase()}</SectionTitle>
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: "1px solid #1a1a1a" }}>
          <div style={{ fontSize: 10, color: "#555", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Meta {q.label}</div>
          <div className="print-value-dark" style={{ fontSize: 20, fontWeight: 700, color: "#f0f0f0", fontFamily: "monospace" }}>{fmt(q.goal)}</div>
        </div>
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: `1px solid ${GREEN}28`, position: "relative" }}>
          <div style={{ paddingRight: 80 }}>
            <div style={{ fontSize: 10, color: "#666", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Realizado {q.label}</div>
            <div className="print-value-green" style={{ fontSize: 20, fontWeight: 700, color: GREEN, fontFamily: "monospace" }}>{fmt(q.realized)}</div>
            <ProgressBar value={qPct} />
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, color: GREEN, fontWeight: 700 }}>{qPct}%</span>
              {q.lastUpdate && <span style={{ fontSize: 9, color: "#2a2a2a", fontStyle: "italic" }}>atualizado {fmtDate(q.lastUpdate)}</span>}
          ...
          <button className="no-print" onClick={() => setModal(i)} style={{ position: "absolute", top: 14, right: 14, background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 9, color: "#888", fontSize: 11, fontWeight: 700, padding: "6px 10px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = "#e53935"; e.currentTarget.style.color = "#ff6f61"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#2a2a2a"; e.currentTarget.style.color = "#888"; }}>
            ✏️ Editar
          </button>
        </div>
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: `1px solid ${RED}28` }}>
          <div style={{ fontSize: 10, color: "#666", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Faltante {q.label}</div>
          <div className="print-value-red" style={{ fontSize: 20, fontWeight: 700, color: qRem === 0 ? GREEN : RED, fontFamily: "monospace" }}>{fmt(qRem)}</div>
        </div>
      </div>
    );
  });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", fontFamily: "'DM Sans', sans-serif", paddingBottom: 60 }}>
      {headerContent}
      <div ref={useRef(null)} className="print-page" style={{ padding: contentPad }}>
        {isMobile && (
          <>
            <SectionTitle>ANUAL</SectionTitle>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: "1px solid #1a1a1a" }}>
              <div style={{ fontSize: 10, color: "#555", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Meta Anual</div>
              <div className="print-value-dark" style={{ fontSize: 22, fontWeight: 800, color: "#f0f0f0", fontFamily: "monospace" }}>{fmt(client.annualGoal)}</div>
            </div>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: `1px solid ${GREEN}28` }}>
              <div style={{ fontSize: 10, color: "#666", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Realizado Anual</div>
              <div className="print-value-green" style={{ fontSize: 22, fontWeight: 800, color: GREEN, fontFamily: "monospace" }}>{fmt(total)}</div>
              <ProgressBar value={annualPct} />
              <span style={{ fontSize: 11, color: GREEN, fontWeight: 700 }}>{annualPct}% atingido</span>
            </div>
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 18px", marginBottom: 10, border: `1px solid ${RED}28` }}>
              <div style={{ fontSize: 10, color: "#666", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>Faltante Anual</div>
              <div className="print-value-red" style={{ fontSize: 22, fontWeight: 800, color: annualRem === 0 ? GREEN : RED, fontFamily: "monospace" }}>{fmt(annualRem)}</div>
            </div>
          </>
        )}
        {!isMobile && (
...
function KpiDashboard({ clients, isMobile, onBack }) {
  ...
  const projAnual = totalRealized * (12 / monthsElapsed);
  const projPct = pct(projAnual, totalGoal);

  const handlePrint = () => {
    const style = document.createElement("style"); style.innerHTML = PRINT_CSS; document.head.appendChild(style);
    window.print(); setTimeout(() => document.head.removeChild(style), 1000);
  };

  const KpiCard = ({ icon, label, value, sub, color }) => (
    <div style={{ background: "#111", borderRadius: 14, padding: "16px 16px 14px", border: `1px solid ${color}22`, flex: "1 1 140px", minWidth: 0 }}>
      <div style={{ fontSize: 20, marginBottom: 6 }}>{icon}</div>
      <div style={{ fontSize: 10, color: "#555", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
    ...
    <div style={{ minHeight: "100vh", background: "#080808", fontFamily: "'DM Sans', sans-serif", paddingBottom: 60 }}>
      <div className="no-print" style={{ background: "#0f0f0f", padding: isMobile ? "44px 22px 20px" : "28px 36px 24px", borderBottom: "1px solid #181818", ...(isMobile ? { position: "sticky", top: 0, zIndex: 10 } : {}) }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontSize: isMobile ? 22 : 26, fontWeight: 800, color: "#f0f0f0", letterSpacing: -0.5 }}>Painel de KPIs</div>
          <div style={{ padding: "4px 10px", background: `${rating.color}18`, borderRadius: 20, fontSize: 11, color: rating.color, fontWeight: 700 }}>{rating.icon} {rating.label}</div>
        </div>
        <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>{clients.length} empresa{clients.length !== 1 ? "s" : ""}</div>
      </div>

      <div className="print-page" style={{ padding: pad }}>
        ...
        <div className="print-card" style={{ background: "linear-gradient(135deg,#160404,#1c0808)", borderRadius: 16, padding: "20px 20px 16px", marginBottom: 14, border: "1px solid #e5393525" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
            <div style={{ fontSize: 28, fontWeight: 900, color: GREEN, fontFamily: "monospace" }}>{fmtShort(totalRealized)}</div>
            <div style={{ fontSize: 14, color: "#555" }}>de {fmtShort(totalGoal)}</div>
          </div>
          <ProgressBar value={totalPct} />
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
            <span style={{ fontSize: 13, color: GREEN, fontWeight: 700 }}>{totalPct}% da meta anual</span>
            <span style={{ fontSize: 11, color: RED }}>faltam {fmtShort(totalRemaining)}</span>
          </div>
        </div>
        ...
        {offTrack > 0 && (
          <div style={{ background: "#150e00", borderRadius: 12, padding: "12px 16px", marginBottom: 14, border: "1px solid #f59e0b33", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 18 }}>⚠️</span>
            <div>
              <div style={{ fontSize: 13, color: AMBER, fontWeight: 700 }}>{offTrack} empresa{offTrack > 1 ? "s" : ""} abaixo de 50% no 1º tri</div>
              <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>Ação comercial recomendada</div>
            </div>
          </div>
        )}
        ...
        {!isMobile ? (
          <div style={{ background: "#111", borderRadius: 16, border: "1px solid #1a1a1a", overflow: "hidden", marginBottom: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "40px 2fr 1fr 1fr 1fr 100px", padding: "10px 20px", borderBottom: "1px solid #1a1a1a" }}>
              {["#", "Empresa", "Meta", "Realizado", "% Anual", "1º Tri"].map(h => (
                <div key={h} style={{ fontSize: 10, color: "#444", letterSpacing: 1.2, textTransform: "uppercase" }}>{h}</div>
              ))}
            </div>
            {rows.map((r, i) => (
              <div key={r.name} style={{ display: "grid", gridTemplateColumns: "40px 2fr 1fr 1fr 1fr 100px", padding: "14px 20px", borderBottom: i < rows.length - 1 ? "1px solid #161616" : "none", alignItems: "center" }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: r.pct >= 80 ? GREEN : r.pct >= 50 ? AMBER : RED, fontFamily: "monospace" }}>{r.pct}%</div>
          ...
          rows.map((r, i) => (
            <div key={r.name} className="print-card" style={{ background: "#111", borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: "1px solid #1a1a1a" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, width: 20 }}>{i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}.`}</div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#f0f0f0" }}>{r.name}</div>
                    <div style={{ fontSize: 10, color: "#555" }}>Meta: {fmtShort(r.annualGoal)}</div>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: r.pct >= 80 ? GREEN : r.pct >= 50 ? AMBER : RED, fontFamily: "monospace" }}>{r.pct}%</div>
                  <div style={{ fontSize: 10, color: GREEN }}>{fmtShort(r.realized)}</div>
                </div>
              </div>
              <ProgressBar value={r.pct} thin />
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span style={{ fontSize: 10, color: "#555" }}>1º Tri: <span style={{ color: r.qPct >= 70 ? GREEN : r.qPct >= 40 ? AMBER : RED, fontWeight: 700 }}>{r.qPct}%</span></span>
                <span style={{ fontSize: 10, color: r.pct >= 80 ? GREEN : r.pct >= 50 ? AMBER : RED }}>{r.pct >= 80 ? "No ritmo ✓" : r.pct >= 50 ? "Atenção" : "Crítico"}</span>
              </div>
            </div>
          ))
        )}
        ...
        <div style={{ display: !isMobile ? "grid" : "block", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", marginBottom: isMobile ? 10 : 0, border: "1px solid #1a1a1a" }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: projPct >= 100 ? GREEN : projPct >= 70 ? AMBER : RED, marginTop: 6 }}>
              {projPct >= 100 ? "✅ Meta atingível" : projPct >= 70 ? "⚡ Precisa acelerar" : "🚨 Ritmo insuficiente"}
            </div>
          </div>
          <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: "16px 18px", border: "1px solid #1a1a1a", marginTop: isMobile ? 0 : 0 }}>
...
export default function App() {
  ...
  const [clients, setClients] = useState(makeClients());
  const [screen, setScreen] = useState("login");
  const [currentUser, setCurrentUser] = useState(null);
  const [selected, setSelected] = useState(null);

  const handleSaveClient = (c) => setClients(prev => { const exists = prev.find(x => x.id === c.id); return exists ? prev.map(x => x.id === c.id ? c : x) : [...prev, c]; });
  const handleDeleteClient = (id) => { setClients(prev => prev.filter(c => c.id !== id)); setUsers(prev => prev.map(u => ({ ...u, companyIds: (u.companyIds || []).filter(cid => cid !== id) }))); if (selected?.id === id) setSelected(null); };
  const handleUpdate = (qIndex, value) => {
    const now = new Date().toISOString();
    const update = c => ({ ...c, quarters: c.quarters.map((q, i) => i === qIndex ? { ...q, realized: value, lastUpdate: now } : q) });
    setClients(prev => prev.map(c => c.id === selected.id ? update(c) : c));
    setSelected(prev => update(prev));
  };
  const handleSelect = (c) => { setSelected(clients.find(x => x.id === c.id) || c); setScreen("dashboard"); };
  const handleUpdateUser = (userId, changes) => { setUsers(prev => prev.map(u => u.id === userId ? { ...u, ...changes } : u)); if (currentUser?.id === userId) setCurrentUser(prev => ({ ...prev, ...changes })); };
  const handleUpdateUserCompanies = (userId, companyIds) => { setUsers(prev => prev.map(u => u.id === userId ? { ...u, companyIds } : u)); if (currentUser?.id === userId) setCurrentUser(prev => ({ ...prev, companyIds })); };
  ...
  const mainContent = (() => {
    ...
    return (
      <ClientList
        clients={clients} currentUser={currentUser} users={users} isMobile={isMobile}
        onSelect={handleSelect} onSaveClient={handleSaveClient} onDeleteClient={handleDeleteClient}
        onAddUser={u => setUsers(p => [...p, u])}
        onDeleteUser={id => setUsers(p => p.filter(u => u.id !== id))}
        onUpdateUser={handleUpdateUser} onUpdateUserCompanies={handleUpdateUserCompanies}
  ...
  return (
    <DesktopLayout currentUser={currentUser} screen={screen} onNav={handleNav} onLogout={() => { setCurrentUser(null); setScreen("login"); }}>
      {mainContent}
    </DesktopLayout>
  );
}
