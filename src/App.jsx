...
function SettingsScreen({ currentUser, appIcon, onUpdateUser, onUpdateAppIcon, isMobile, onBack }) {
  ...
  const avRef = useRef(); const icoRef = useRef();
  ...
  const saveProfile = () => {
    ...
    flash(true, "Perfil atualizado com sucesso!"); setPass(""); setPass2("");
  };
        ...
        <div style={{ background: "#111", borderRadius: 16, border: "1px solid #1a1a1a", overflow: "hidden" }}>
          ...
          {visibleClients.map((c, idx) => { const total = c.quarters.reduce((s, q) => s + q.realized, 0); const p = pct(total, c.annualGoal); return (
            <div key={c.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.5fr 100px 120px", padding: "14px 20px", borderBottom: idx < visibleClients.length - 1 ? "1px solid #161616" : "none", alignItems: "center", transition: "background .15s" }} onMouseEnter={e => e.currentTarget.style.background = "#161616"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <div style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }} onClick={() => onSelect(c)}><Avatar name={c.name} size={36} fs={16} /><div><div style={{ fontSize: 14, fontWeight: 700, color: "#f0f0f0" }}>{c.name}</div><div style={{ fontSize: 11, color: "#444" }}>{c.year}</div></div></div>
  ...
  const content = (
    <div style={{ padding: isMobile ? "16px 18px 80px" : "0 36px 60px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(3,1fr)", gap: isMobile ? 10 : 14, marginBottom: 8 }}>
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: isMobile ? "14px 14px" : "18px 20px", border: "1px solid #1a1a1a" }}>
        ...
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: isMobile ? "14px 14px" : "18px 20px", border: `1px solid ${GREEN}28`, gridColumn: isMobile ? "span 2" : "auto" }}>
          ...
          <ProgressBar value={annualPct} print={false} />
          <div style={{ fontSize: 12, color: GREEN, fontWeight: 700 }} className="print-value-green">{annualPct}% atingido</div>
        </div>
        <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: isMobile ? "14px 14px" : "18px 20px", border: `1px solid ${RED}28` }}>
      ...
      {quarterCards.map(({ q, i, qPct, qRem }) => (
        <div key={i}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(3,1fr)", gap: isMobile ? 10 : 14, marginBottom: 8 }}>
            ...
            <div className="print-card" style={{ background: "#111", borderRadius: 14, padding: isMobile ? "14px 14px" : "18px 20px", border: `1px solid ${GREEN}28`, gridColumn: isMobile ? "span 2" : "auto", position: "relative" }}>
...
function KpiDashboard({ clients, isMobile, onBack }) {
  ...
  const q1Goal = clients.reduce((s, c) => s + (c.quarters[0]?.goal || 0), 0);
  const q1Real = clients.reduce((s, c) => s + (c.quarters[0]?.realized || 0), 0);
  ...
  const onTrack = clients.filter(c => pct(c.quarters[0]?.realized || 0, c.quarters[0]?.goal || 1) >= 50).length;
  ...
  const rows = clients.map(c => { const r = c.quarters.reduce((s, q) => s + q.realized, 0); const p = pct(r, c.annualGoal); const qP = pct(c.quarters[0]?.realized || 0, c.quarters[0]?.goal || 0); return { name: c.name, annualGoal: c.annualGoal, realized: r, pct: p, qPct: qP }; }).sort((a, b) => b.pct - a.pct);
  const handlePrint = () => { const style = document.createElement("style"); style.innerHTML = PRINT_CSS; document.head.appendChild(style); window.print(); setTimeout(() => document.head.removeChild(style), 1000); };
    ...
    <div className="print-page" style={{ minHeight: "100vh", background: "#080808", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: isMobile ? "16px 18px 80px" : "0 36px 60px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(3,1fr)", gap: isMobile ? 10 : 14, marginBottom: 14 }}>
          {[
            ...
            <div key={k.label} className="print-card" style={{ background: "#111", borderRadius: 14, padding: isMobile ? "14px" : "16px 18px", border: `1px solid ${k.c}22` }}>
              ...
              {k.sub && <div style={{ fontSize: 10, color: "#444", marginTop: 3 }}>{k.sub}</div>}
            </div>
          ))}
        </div>
        ...
        <div style={{ background: "#111", borderRadius: 16, border: "1px solid #1a1a1a", overflow: "hidden", marginBottom: 14 }}>
          {!isMobile && <div style={{ display: "grid", gridTemplateColumns: "40px 2fr 1.2fr 1.2fr 1fr", padding: "10px 20px", borderBottom: "1px solid #1a1a1a" }}>
            {["#", "Empresa", "Meta Anual", "Realizado", "% Anual"].map(h => <div key={h} style={{ fontSize: 10, color: "#444", letterSpacing: 1.2, textTransform: "uppercase" }}>{h}</div>)}
          </div>}
          {rows.map((r, i) => (
            <div key={r.name} className="print-card" style={{ display: "grid", gridTemplateColumns: isMobile ? "36px 1fr" : "40px 2fr 1.2fr 1.2fr 1fr", padding: isMobile ? "12px 16px" : "14px 20px", borderBottom: i < rows.length - 1 ? "1px solid #161616" : "none", alignItems: "center", gap: isMobile ? 10 : 0 }}>
              ...
              {isMobile ? (
                ...
                <>
                  ...
                  <div><div style={{ fontSize: 15, fontWeight: 800, color: r.pct >= 80 ? GREEN : r.pct >= 50 ? AMBER : RED, fontFamily: "monospace", marginBottom: 3 }}>{r.pct}%</div><ProgressBar value={r.pct} thin /></div>
                </>
              )}
            </div>
          ))}
...
export default function App() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (settingsData) { const iconRow = settingsData.find(r => r.key === "app_icon"); if (iconRow?.value) setAppIcon(iconRow.value); }
      setLoading(false);
    }
    loadData();
  }, []);

  // ── Realtime: atualiza companies em tempo real ──
  useEffect(() => {
  ...
  const handleNav = (target) => { if (target === "kpi") setScreen("kpi"); else if (target === "settings") setScreen("settings"); else setScreen("clients"); };

  const kpiClients = currentUser?.isAdmin ? clients : clients.filter(c => (currentUser?.companyIds || []).includes(c.id));
